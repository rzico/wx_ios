...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    if (_showTakePhotoBtn) {
        ...
        }else if(tzImagePickerVc.allowPickingVideo && tzImagePickerVc.allowTakePicture){
            return _models.count + 1;
        }
    }
...
//            [mediaTypes addObject:(NSString *)kUTTypeVideo];
//        }
//        self.imagePickerVc.mediaTypes = @[(NSString *)kUTTypeVideo,(NSString *)kUTTypeImage];
//
//        if (tzImagePickerVc.allowPickingImage){
...
//        }
        ...
        for (NSString *type in availableMediaTypes){
            if ([type containsString:@"image"] && tzImagePickerVc.allowPickingImage){
                [mediaTypes addObject:type];
            }else if ([type containsString:@"movie"] && tzImagePickerVc.allowPickingVideo){
                [mediaTypes addObject:type];
            }
        }
        
        [[self imagePickerVc] setMediaTypes:mediaTypes];
        
        
        self.imagePickerVc.videoQuality = UIImagePickerControllerQualityTypeHigh;
        self.imagePickerVc.cameraCaptureMode = tzImagePickerVc.allowPickingVideo ? UIImagePickerControllerCameraCaptureModeVideo : UIImagePickerControllerCameraCaptureModePhoto;
