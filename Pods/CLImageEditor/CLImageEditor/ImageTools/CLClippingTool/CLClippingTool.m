...
+ (NSArray*)defaultPresetRatios
{
    return @[
             ...
             @{kCLClippingToolRatioValue1:@1, kCLClippingToolRatioValue2:@2, kCLClippingToolRatioTitleFormat:@"%g : %g"},
             @{kCLClippingToolRatioValue1:@2, kCLClippingToolRatioValue2:@3, kCLClippingToolRatioTitleFormat:@"%g : %g"},
             @{kCLClippingToolRatioValue1:@3, kCLClippingToolRatioValue2:@4, kCLClippingToolRatioTitleFormat:@"%g : %g"},
             ];
}
...
- (void)setup
{
    ...
    if(!swapBtnHidden){
        ...
        btn.center = CGPointMake(btnPanel.width/2, (btnPanel.height - 20)/2);
        ...
        UILabel *titleLable = [[UILabel alloc] initWithFrame:CGRectMake(0, btn.bottom + 5, buttonWidth, 15)];
        titleLable.backgroundColor = [UIColor clearColor];
        titleLable.textColor = [CLImageEditorTheme toolbarTextColor];
        titleLable.font = [CLImageEditorTheme toolbarTextFont];
        titleLable.textAlignment = NSTextAlignmentCenter;
        titleLable.text = @"旋转";
        [btnPanel addSubview:titleLable];
    }
...
- (void)setCropMenu
{
    ...
    UIImage *iconImage = [self.editor.imageView.image resize:CGSizeMake(30 * imgSize.width/maxW, 30 * imgSize.height/maxW)];
    ...
    for(NSDictionary *info in ratios){
        ...
        CGFloat diff = _menuScroll.frame.size.width - ratios.count * W;
        if (0 < diff){
...
- (void)refreshViews
{
    ...
    if(_ratio.ratio!=0){
        ...
        W = 30 * _iconView.image.size.width / maxW;
        H = 30 * _iconView.image.size.height / maxW;
    }
