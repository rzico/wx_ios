...
- (instancetype)init
{
    if (self = [super init])
    {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(SystemFaceSendButtonOnTouched:) name:@"SystemFaceSendButtonOnTouched" object:nil];
    }
    return self;
}
...
- (void)layoutSubviews
{
    //重画ChatInputPanel 位置 by cj
    if ([UIDevice isIphoneX]){
        CGRect frame = self.frame;
        //orgin.y = screenHeight - statusBarHeight - bottomHeight - navigationBarHeight - selfFrameHeight
        frame.origin = CGPointMake(0, getScreenHeight() - [UIApplication sharedApplication].statusBarFrame.size.height - 34 - 44 - self.height);
        self.frame = frame;
    }
...
//by cj
- (void)SystemFaceSendButtonOnTouched:(NSNotification *)notification{
    [(RichChatInputToolBar *)_toolBar SystemFaceSendButtonOnTouched];
}
@end
