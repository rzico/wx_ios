    ...
    else
    {
        if (_datas.count == 0)
        {
            [HUDHelper alert:@"没有新朋友消息" action:^{
                [SharedAppDelegate popViewController];
            }];
        }
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([item isMyFriend])
    {
        ...
        [SharedAppDelegate pushViewController:pro withBackTitle:@"返回"];
    }
    else if ([item isRecormend])
    {
        ...
        vc.title = @"添加好友";
        [SharedAppDelegate pushViewController:vc withBackTitle:@"返回"];
    ...
    else if ([item isPendency])
    {
        ...
        else
        {
            ...
            [SharedAppDelegate pushViewController:vc];
        }
    }
