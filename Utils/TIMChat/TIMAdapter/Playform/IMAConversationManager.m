...
{
    //收到消息 播放提示音  by cj
    [SharedAppDelegate playSoundOnNewMessage];
...
- (void)updateOnLastMessageChanged:(IMAConversation *)conv
{
    if ([_chattingConversation isEqual:conv])
    {
        ...
        {
            //更改插入位置  by cj
            NSInteger insertIndex = _conversationList.count > 0 ? [self insertPosition] : 0;
            [_conversationList insertObject:conv atIndex:insertIndex];
            [self updateOnNewConversation:conv];
        }
        else
        {
