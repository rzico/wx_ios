...
typedef NS_ENUM (NSInteger, ToolBarTag){
    ToolBarTagBlack = 0x000000 + 100,
    ToolBarTagRed = 0xff0000 + 100,
    ToolBarTagBlue = 0x0000ff + 100,
    ToolBarTagBold = 200,
    ToolBarTagItalic = 201,
    ToolBarTagUnderline = 202,
    ToolBarTagFontH2 = 400,
    ToolBarTagFontH3 = 401,
    ToolBarTagFontH4 = 402,
...
- (void)viewDidLoad {
    ...
    if (frame.size.height == [[UIScreen mainScreen] bounds].size.height){
        frame.origin.y = 64;
        frame.size.height = [[UIScreen mainScreen] bounds].size.height - 64 - 88;
    }
...
- (void)createParentHoldingView:(CGRect)frame {
    ...
    CGFloat originY = self.view.frame.size.height - frame.origin.y;
    ...
    if (_alwaysShowToolbar) {
        self.toolbarHolder.frame = CGRectMake(0, originY - 120, self.view.frame.size.width, height);
    } else {
        self.toolbarHolder.frame = CGRectMake(0, originY, self.view.frame.size.width, height);
    }
...
//    for (int i = 0; i < 3; i ++){
//        left += W * size[i];
//        UIView *line = [[UIView alloc] initWithFrame:CGRectMake(left - 0.5, H * 0.06, 1, 88 * 0.88)];
//        line.backgroundColor = [UIColor colorWithHex:0xbbbcbc];
//        [self.toolbarHolder addSubview:line];
//    }
    
    UIButton *selAllbutton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    selAllbutton.clipsToBounds = YES;
    [selAllbutton setFrame:CGRectMake(0, 0, 50, 25)];
    ...
    [selAllbutton addTarget:self action:@selector(selectAll) forControlEvents:UIControlEventTouchUpInside];
    [selAllbutton.layer setCornerRadius:5];
    [selAllbutton setBackgroundColor:[UIColor colorWithRed:0x1c/255.0 green:0x73/255.0 blue:0xd3/255.0 alpha:1]];
    selAllbutton.center = CGPointMake((W * 2)/2.0, H / 2.0);
    [self.toolbarHolder addSubview:selAllbutton];
    
    int colorArray[] = {0x000000/*黑*/,
                        0xff0000/*红*/,
                        0x0000ff/*蓝*/};
    
    NSArray *colorNameArray = [NSArray arrayWithObjects:@"black",
                                                        @"red",
                                                        @"blue",nil];
    ...
    for (int i = 0; i < 3; i ++){
        UIButton *button = [self createToolBarButtonWithTag:100 + colorArray[i]  andLeft:left andTop:(H - W)/2.0 andWidth:W andSel:@selector(changeTextColor:)];
        [button setImage:[UIImage imageNamed:[colorNameArray objectAtIndex:i]] forState:UIControlStateNormal];
        left += W;
    }
    ...
    for (int i = 0; i < 3; i ++){
        UIButton *button = [self createToolBarButtonWithTag:200 + i andLeft:left andTop:(H - W)/2.0 andWidth:W andSel:@selector(changeFontStyle:)];
        [button setImage:[UIImage imageNamed:[fontStyleNameArray objectAtIndex:i]] forState:UIControlStateNormal];
        [button setImage:[UIImage imageNamed:[[fontStyleNameArray objectAtIndex:i] stringByAppendingString:@"_SEL"]] forState:UIControlStateSelected];
        left += W;
    }
    ...
    for (int i = 0; i < 2; i ++){
        UIButton *button = [self createToolBarButtonWithTag:300 + i andLeft:left andTop:H + (H - W)/2.0 andWidth:W andSel:@selector(doFun:)];
        [button setImage:[UIImage imageNamed:[doNameArray objectAtIndex:i]] forState:UIControlStateNormal];
        left += W;
    }
    
    
    NSArray<NSString *> *fontSizeArray = [[NSArray<NSString *> alloc] initWithObjects:@"ZSSh1",@"ZSSh2",@"ZSSh3",nil];
    for (int i = 0; i < 3; i ++){
        UIButton *button = [self createToolBarButtonWithTag:400 + i andLeft:left andTop:H + (H - W)/2.0 andWidth:W andSel:@selector(changeFontSize:)];
        [button setImage:[UIImage imageNamed:[fontSizeArray objectAtIndex:i]] forState:UIControlStateNormal];
        [button setImage:[UIImage imageNamed:[[fontSizeArray objectAtIndex:i] stringByAppendingString:@"_SEL"]] forState:UIControlStateSelected];
        left += W;
    }
    ...
    for (int i = 0; i < 3; i ++){
        UIButton *button = [self createToolBarButtonWithTag:500 + i andLeft:left andTop:H + (H - W)/2.0 andWidth:W andSel:@selector(changeTextAlign:)];
        [button setImage:[UIImage imageNamed:[textAlignArray objectAtIndex:i]] forState:UIControlStateNormal];
        [button setImage:[UIImage imageNamed:[[textAlignArray objectAtIndex:i] stringByAppendingString:@"_SEL"]] forState:UIControlStateSelected];
        left += W;
    }
    
    UIButton *hideKeyboard = [self createToolBarButtonWithTag:600 andLeft:0 andTop:0 andWidth:W andSel:@selector(dismissKeyboard)];
    hideKeyboard.center = CGPointMake(self.toolbarHolder.width - W / 2.0, 44);
    [hideKeyboard setImage:[UIImage imageNamed:@"ZSSkeyboard"] forState:UIControlStateNormal];
    [hideKeyboard setImageEdgeInsets:UIEdgeInsetsZero];
...
//    [self.toolbarHolder addSubview:view];
    ...
    [button setImageEdgeInsets:UIEdgeInsetsMake(width * 0.7, width * 0.7, width * 0.7, width * 0.7)];
    button.imageView.layer.cornerRadius = 5;
    [button addTarget:self action:sel forControlEvents:UIControlEventTouchUpInside];
//    [button setBackgroundImage:[UIImage imageWithColor:[UIColor colorWithHex:0x1c73d3]] forState:UIControlStateSelected];
    [button setBackgroundImage:[UIImage createImageWithColor:[UIColor colorWithHex:UINavigationBarColor] frame:button.frame] forState:UIControlStateSelected];
...
- (void)changeTextColor:(UIButton *)button{
    ...
    [self setSelectedColor:[UIColor colorWithHex:(int)(button.tag - 100)] tag:1];
}
...
//    }
    ...
    NSString *trigger = [NSString stringWithFormat:@"zss_editor.setHeading('h%zd');", size];
...
- (void)updateToolBarWithButtonName:(NSString *)name {
    ...
    // Highlight items
    UIButton *btn;
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagBold];
    [btn setSelected:[itemNames containsObject:@"bold"]];
    
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagItalic];
    [btn setSelected:[itemNames containsObject:@"italic"]];
    
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagUnderline];
    [btn setSelected:[itemNames containsObject:@"underline"]];
...
//    [self setView:[self.view viewWithTag:ToolBarTagUnderline] backgroundColor:![itemNames containsObject:@"underline"]];
    ...
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagBlack];
    [btn setSelected:[itemNames containsObject:@"#000000"]];
    
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagRed];
    [btn setSelected:[itemNames containsObject:@"#FF0000"]];
    
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagBlue];
    [btn setSelected:[itemNames containsObject:@"#0000FF"]];
...
//    [self setView:[self.view viewWithTag:ToolBarTagBlue] backgroundColor:![itemNames containsObject:@"#0000FF"]];
    ...
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagAlignLeft];
    [btn setSelected:[itemNames containsObject:@"justifyLeft"]];
    
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagAlignCenter];
    [btn setSelected:[itemNames containsObject:@"justifyCenter"]];
    
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagAlignRight];
    [btn setSelected:[itemNames containsObject:@"justifyRight"]];
...
//    [self setView:[self.view viewWithTag:ToolBarTagAlignRight] backgroundColor:![itemNames containsObject:@"justifyRight"]];
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagFontH2];
    [btn setSelected:[itemNames containsObject:@"h4"]];
    
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagFontH3];
    [btn setSelected:[itemNames containsObject:@"h3"]];
    
    btn = (UIButton *)[self.view viewWithTag:ToolBarTagFontH4];
    [btn setSelected:[itemNames containsObject:@"h2"]];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    if (self.shouldShowKeyboard) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            NSString *trigger = [NSString stringWithFormat:@"zss_editor.setHeading('h3');"];
            [self.editorView stringByEvaluatingJavaScriptFromString:trigger];
        });
    }
...
- (void)keyboardWillShowOrHide:(NSNotification *)notification {
    ...
    if ([notification.name isEqualToString:UIKeyboardWillShowNotification]) {
        ...
        [UIView animateWithDuration:duration delay:0 options:animationOptions animations:^{
            ...
            if (self.view.height == [UIScreen mainScreen].bounds.size.height){
                frameHeight -= 64;
            }
            
            CGRect editorFrame = self.editorView.frame;
            editorFrame.size.height = (frameHeight - keyboardHeight) - sizeOfToolbar - extraHeight;
            ...
            sourceFrame.size.height = (frameHeight - keyboardHeight) - sizeOfToolbar - extraHeight;
        ...
        [UIView animateWithDuration:duration delay:0 options:animationOptions animations:^{
            ...
            if (_alwaysShowToolbar) {
                editorFrame.size.height = ((frameHeight - sizeOfToolbar) - extraHeight);
            } else {
                editorFrame.size.height = frameHeight;
            }
            ...
            if (_alwaysShowToolbar) {
                sourceFrame.size.height = ((frameHeight - sizeOfToolbar) - extraHeight);
            } else {
                sourceFrame.size.height = frameHeight;
            }
