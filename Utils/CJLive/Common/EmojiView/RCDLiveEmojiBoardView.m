...
//        RCDLive_DebugLog(@"Emoji.plist > %@", bundlePath);
        ...
        self.emojiBackgroundView.backgroundColor = [UIColor colorWithHex:0xf8f8f8];
...
- (void)enableSendButton:(BOOL)sender{
    ...
    if (sender) {
        ...
        sendButton.backgroundColor = [UIColor colorWithHex:0x0099ff];
        [sendButton  setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    }else{
        sendButton.userInteractionEnabled = NO;
        sendButton.backgroundColor = [UIColor colorWithHex:0xfafafa];
        [sendButton  setTitleColor:[UIColor colorWithHex:0x999999] forState:UIControlStateNormal];
    }
    ...
    for (int i = beginEmojiBtn; i < endEmojiBtn; i++) {
       ...
       // NSLog(@"<string>%@</string>", faceEmojiArray[i]);
        ...
        if (((i + 1) >= self.emojiMaxCountPerPage && (i + 1) % self.emojiMaxCountPerPage == 0) ||
            ...
            [deleteButton setImage:[UIImage imageNamed:@"emoji_btn_delete"] forState:UIControlStateNormal];
...
-(void)loadCustomerEmoticonPackage{
    ...
    emojiBtn.backgroundColor = [UIColor colorWithHex:0xf8f8f8];
    emojiBtn.frame = CGRectMake(0,5, 45, 37);
    [emojiBtn setImage:[UIImage imageNamed:@"emoji_btn_normal"] forState:UIControlStateNormal];
...
- (void)tabBtnHandle:(UIButton *)sender {
    for (int i = 0;i < self.tabIconArray.count;i++) {
        ...
        if (sender == btn) {
            btn.backgroundColor = [UIColor colorWithHex:0xf8f8f8];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    tabBtn.backgroundColor = [UIColor colorWithHex:0xf8f8f8];
    pageCtrl.numberOfPages = self.emojiTotalPage;
