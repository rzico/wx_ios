...
@property (weak, nonatomic) IBOutlet OLImageView *gift;
...
- (void)setModel:(PresentModel *)model
{
    ...
    if (model.icon != nil) {
        ...
        self.icon.image = [UIImage imageNamed:@"newDefaultHead"];
    }
    self.senderName.text    = model.sender;
    self.giftNameLable.text = [NSString stringWithFormat:@"送了一个【%@】", model.giftName];
//    self.gift.image         = [OLImage imageNamed:model.giftImageName];
    self.gift.image         = [OLImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:model.giftImageName]]];
}
