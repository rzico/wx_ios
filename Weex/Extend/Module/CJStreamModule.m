...
- (void)fetch:(NSDictionary *)options callback:(WXModuleCallback)callback progressCallback:(WXModuleKeepAliveCallback)progressCallback
{
    ...
    loader.onResponseReceived = ^(const WXResourceResponse *response) {
        ...
        if (weakSelf) {
            ...
            statusText = [CJStreamModule _getStatusText:httpResponse.statusCode];
    ...
    loader.onFinished = ^(const WXResourceResponse * response, NSData *data) {
        if (weakSelf && callback) {
            ...
            if (response){
                ...
                if ([response.allHeaderFields objectForKey:@"statusText"]){
            ...
            if (![callbackRsp objectForKey:@"data"] || ![[callbackRsp objectForKey:@"data"] isKindOfClass:[NSDictionary class]]){
                NSMutableDictionary *dic = [NSMutableDictionary new];
                [dic setObject:@"error" forKey:@"type"];
                [dic setObject:@"数据包无效" forKey:@"content"];
                [dic setObject:@"" forKey:@""];
                [callbackRsp setObject:dic forKey:@"data"];
